...
class UserControllerTest {
    ...
    void testCreateUser_success() {
        User user = new User("조기환", "1234");

        ResponseEntity<User> response = restTemplate.postForEntity("/users", user, User.class);

        assertEquals(200, response.getStatusCode().value());
        assertNotNull(response.getBody());
        assertEquals("조기환", response.getBody().getUsername());
    }
    ...
    void testGetAllUsers_withUsers() {
        // 사용자 두 명 등록
        restTemplate.postForEntity("/users", new User("조기환", "1234"), User.class);
        restTemplate.postForEntity("/users", new User("박명수", "1234"), User.class);
        ...
        assertEquals("조기환", response.getBody()[0].getUsername());
        assertEquals("박명수", response.getBody()[1].getUsername());
    }
    ...
    void testGetUserById_success() {
        ResponseEntity<User> createResponse = restTemplate.postForEntity("/users", new User("박명수", "1234"), User.class);
        Long id = createResponse.getBody().getId();

        ResponseEntity<User> response = restTemplate.getForEntity("/users/" + id, User.class);
        assertEquals(200, response.getStatusCode().value());
        assertNotNull(response.getBody());
        assertEquals("박명수", response.getBody().getUsername());
    }
    ...
    void testUpdateUser_success() {
        // 사용자 등록
        User user = new User("조기환", "1234");
        ResponseEntity<User> createResoponse = restTemplate.postForEntity("/users", user, User.class);
        Long id = createResoponse.getBody().getId();

        // 사용자 수정
        User updatedUser = new User("조수환", "5678");
        ...
        assertEquals(200, response.getStatusCode().value());
        assertNotNull(response.getBody());
        assertEquals("조수환", response.getBody().getUsername());
    ...
    void testUpdateUser_notFound() { // 업데이트 하려고 하는데 해당 id의 유저가 없을 때
        // 업데이트 할 유저 생성
        User updatedUser = new User("조기환", "5678");
